%define sfos_release %(version | grep -o '\\([0-9]\\+\\.\\?\\)\\+' | cut -f -2 -d '.')

%if "%{getenv:LATEST_RELEASE}"
  %define sfos_release_comp >=
%else
  %define sfos_release_comp =
%endif
...
Requires:       sailfish-version %{sfos_release_comp} %{sfos_release}
